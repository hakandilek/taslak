...
import org.xmdl.mesken.service.UserSecurityAdvice;
...
public class UserSecurityAdviceTest extends MockObjectTestCase {
    ...
    public void testAddUserAsAdmin() throws Exception {
        ...
        user.setId(2L);
        user.setPassword("password");
        user.addRole(new Role(MeskenConstants.ADMIN_ROLE));
        UsernamePasswordAuthenticationToken token =
                new UsernamePasswordAuthenticationToken(user.getUsername(), user.getPassword(), user.getAuthorities());
        token.setDetails(user);
        context.setAuthentication(token);
    ...
    public void testUpdateUserProfile() throws Exception {
        ...
        User user = new User("user");
        user.setId(1L);
        user.getRoles().add(new Role(MeskenConstants.USER_ROLE));
    ...
    public void testChangeToAdminRoleFromUserRole() throws Exception {
        ...
        User user = new User("user");
        user.setId(1L);
        user.getRoles().add(new Role(MeskenConstants.ADMIN_ROLE));
    ...
    public void testAddUserRoleWhenHasAdminRole() throws Exception {
        ...
        user1.addRole(new Role(MeskenConstants.ADMIN_ROLE));
        UsernamePasswordAuthenticationToken token =
